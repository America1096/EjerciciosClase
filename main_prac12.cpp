...
#define MAX_FRAMES 20 //Número máximo de keyframes
...
typedef struct _frame
{
	...
	float incX;		//Variable para IncrementoX, para el frame siguiente 
	float incY;		//Variable para IncrementoY
	float incZ;		//Variable para IncrementoZ
	float rotRodIzq;
	float rotRodDer;
	float rotBrazoIzq;
	float rotBrazoIncIzq;
	float rotBrazoDer;
	float rotBrazoIncDer;
	float rotCabeza;
	float rotIncCabeza;
	float rotInc;
	float rotIncDer;
...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].posX = posX; //Se pasa a la estructura
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].rotBrazoIncIzq = (KeyFrame[playIndex + 1].rotBrazoIzq - KeyFrame[playIndex].rotBrazoIzq) / i_max_steps;
	KeyFrame[playIndex].rotBrazoIncDer = (KeyFrame[playIndex + 1].rotBrazoDer - KeyFrame[playIndex].rotBrazoDer) / i_max_steps;
...
void animate(void)
{
	if (play)
	{
		...
		else
		{
			...
			rotBrazoIzq += KeyFrame[playIndex].rotBrazoIncIzq;
			rotBrazoDer += KeyFrame[playIndex].rotBrazoIncDer;
...
int main()
{
	...
	for (int i = 0; i < MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].rotBrazoIncIzq = 0;
		KeyFrame[i].rotBrazoDer = 0;
		KeyFrame[i].rotBrazoIncDer = 0;
